	...
	return &BuildServer{
		...
		lru: lru.NewLRU(binaryCacheSize, func(hash, rrBinPath string) {
			log.Info("evicting binary cache entry",
				zap.String("hash", hash), zap.String("path", rrBinPath))
			if err := os.RemoveAll(rrBinPath); err != nil {
				log.Error("removing cached binary", zap.String("path", rrBinPath), zap.Error(err))
			}
			tempDir := filepath.Join(os.TempDir(), hash)
			if err := os.RemoveAll(tempDir); err != nil {
				log.Error("removing temp dir", zap.String("path", tempDir), zap.Error(err))
			}
		}, binaryCacheTTL),
		currentlyProcessing: lru.NewLRU(processingLockSize, func(key string, _ struct{}) {
			log.Info("releasing in-flight lock", zap.String("key", key))
		}, processingLockTTL),
...
func (b *BuildServer) Build(ctx context.Context, req *connect.Request[requestV1.BuildRequest]) (*connect.Response[responseV1.BuildResponse], error) {
	...
	b.log.Debug("cache key computed", zap.String("hash", hash))
	...
	if cached, ok := b.lru.Get(hash); ok && !req.Msg.GetForceRebuild() {
		b.log.Debug("cache hit", zap.String("hash", hash))
	...
	gh := github.NewClient("", os.Getenv("GITHUB_TOKEN"), b.rrCache, b.log.Named("GitHub"))
